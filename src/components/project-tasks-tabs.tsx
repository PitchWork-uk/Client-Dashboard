...
export function ProjectTasksTabs({
  ...
  const reviewTasks = localTasks.filter(
  ...
  const handleRevise = async () => {
    ...
    try {
      const response = await fetch("/api/update-task-status", {
        ...
        body: JSON.stringify({
          uniqueIdNumber:
            reviseTask.uniqueIdNumber?.toString() || reviseTask.id,
    ...
    cell: ({ row }: { row: { original: TaskRow } }) => {
      ...
      return (
        <div className="flex items-center gap-4">
          ...
          />
          <Dialog
            open={isReviseDialogOpen && reviseTask?.id === task.id}
            onOpenChange={(open) => {
              if (!open) {
                setIsReviseDialogOpen(false);
                setReviseTask(null);
          ...
          >
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Are you sure you want to request a revision for this task?
                  This will change the status to &ldquo;Revision&ldquo;.
                </DialogDescription>
              </DialogHeader>
              <div className="py-4">

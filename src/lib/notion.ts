    ...
    .map((page) => {
      ...
      return {
        id: (
          ...
          : "",
        uniqueIdNumber: (
          p.properties["ID"] as { unique_id?: { number?: number } }
        )?.unique_id?.number,
        title:
          (p.properties["Title"] as { title?: Array<{ plain_text?: string }> })
            ?.title?.[0]?.plain_text || "",
        project:
          (
            p.properties["Project Name"] as {
              rollup?: {
                array?: Array<{ title?: Array<{ plain_text?: string }> }>;
              };
            }
          )?.rollup?.array?.[0]?.title?.[0]?.plain_text || "",
        type:
          (
            p.properties["Type"] as {
              select?: { name?: string; color?: string };
            }
          )?.select?.name || "",
        typeColor:
          colorPresets[
            (p.properties["Type"] as { select?: { color?: string } })?.select
              ?.color || "default"
          ],
        priority:
          (
            p.properties["Priority"] as {
              select?: { name?: string; color?: string };
            }
          )?.select?.name || "",
        priorityColor:
          colorPresets[
            (p.properties["Priority"] as { select?: { color?: string } })
              ?.select?.color || "default"
          ],
        status:
          (
            p.properties["Status"] as {
              status?: { name?: string; color?: string };
            }
          )?.status?.name || "",
        statusColor:
          colorPresets[
            (p.properties["Status"] as { status?: { color?: string } })?.status
              ?.color || "default"
          ],
        date: (
          p.properties["Date"] as { date?: { start?: string; end?: string } }
        )?.date?.start
          ? `${formatDate(
              (
                p.properties["Date"] as {
        ...
        submittedBy:
          (
            p.properties["Dashboard submission"] as {
...
export async function getProjectsByClientName(
  databaseId: string,
  clientEmail: string
) {
  const response = await notion.databases.query({
...
export async function getTaskCountsByClientId(
  databaseId: string,
  clientId: string
) {
...
export async function updateTaskStatusByUniqueId(
  databaseId: string,
...
export async function getCommentsByTaskId(
  ...
  try {
    ...
    return commentsResponse.results.map((comment) => {
      ...
      const author =
        (comment.created_by as { name?: string })?.name || "Unknown User";
